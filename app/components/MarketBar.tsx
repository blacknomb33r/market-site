...
// Feste User-Zeitzone (Drop-down kommt später)
const USER_TZ = 'Europe/Berlin';

type Market = {
  name: string;
  tz: string;
  open: [number, number];   // [HH, MM] in Markt-Lokalzeit
  close: [number, number];  // [HH, MM] in Markt-Lokalzeit
...
function nextValidDay(dt: DateTime, validDays: number[]) {
  for (let i=1; i<=7; i++) {
    ...
    if (validDays.includes((cand.weekday % 7))) return cand;
  }
...
export default function MarketBar() {
  ...
  const views = useMemo<View[]>(() => {
    const now = DateTime.now(); // lokale Zeit (irrelevant, wir setzen Zonen explizit)
    return MARKETS.map(m => {
      if (m.alwaysOpen) {
        // Öffnungszeiten in User-Zone zeigen:
        const openLocal  = now.setZone(m.tz).set({ hour: m.open[0], minute: m.open[1], second: 0, millisecond: 0 }).setZone(USER_TZ);
        const closeLocal = now.setZone(m.tz).set({ hour: m.close[0], minute: m.close[1], second: 0, millisecond: 0 }).setZone(USER_TZ);
        return {
          name: m.name,
          isOpen: true,
          statusText: 'Offen',
      ...
      const isTradingDay = m.days.includes((nowMkt.weekday % 7));
      const openToday  = nowMkt.set({ hour: m.open[0], minute: m.open[1], second: 0, millisecond: 0 });
      const closeToday = nowMkt.set({ hour: m.close[0], minute: m.close[1], second: 0, millisecond: 0 });

      const openLocal  = openToday.setZone(USER_TZ);
      const closeLocal = closeToday.setZone(USER_TZ);
      const hoursLocal = `${formatHM(openLocal)}–${formatHM(closeLocal)} (deine Zeit)`;
      ...
      if (isTradingDay && nowMkt >= openToday && nowMkt <= closeToday) {
        ...
        if (!(isTradingDay && nowMkt < openToday)) {
          ...
          nextOpen = nd.set({ hour: m.open[0], minute: m.open[1], second: 0, millisecond: 0 }).setZone(m.tz);
        }
